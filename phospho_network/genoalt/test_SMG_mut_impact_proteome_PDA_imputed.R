...
# source("./phospho-signaling-master/phospho_network/phospho_network_shared.R")
...
SMG_list <- c("KRAS","TP53","SMAD4","CDKN2A","RNF43","ARID1A","TGFBR2","GNAS","RREB1","PBRM1","BRAF","CTNNB1","U2AF1","KDM6A")
Most_mutated_list <- c("DNAH14", "DNAH9", "FMN2", "METTL4", "MYT1L", "OBSCN", "PLXNA2", "RYR3", "TAF1L", "MUC16", "RNF43", "RYR2", "ZFHX4", "CTNNA2", "MUC4", "TTN", "CDKN2A", "SMAD4", "TP53", "KRAS")
pair_tab <- pair_tab_annotated[pair_tab_annotated$GENE %in% SMG_list, c("GENE", "SUB_GENE")]



#affected_exp_data <- fread("./cptac2p_analysis/inputs/PDA_76BulkDIA_proteome_pqn_log2_NArm_Tumor.tsv", data.table = F)
#affected_exp_data <- fread("./cptac2p_analysis/inputs/PDA_76BulkDIA_tumor_core_normal_proteome_pqn_log2_NArm10_AllSamples_imputed.tsv", data.table = F)
#affected_exp_data <- fread("./cptac2p_analysis/inputs/PDA_76BulkDIA_tumor_core_normal_proteome_pqn_log2_NArm10_AllSamples_imputed_tumoronly.tsv", data.table = F)
  ...
  for (variant_class in variant_classes2process) {
    for (cancer in cancers2process) {
      ...
      if (!file.exists(fn)) {
        ...
        maf <- fread(input = "./cptac2p_analysis/inputs/77_PDA_bulk.withmutect.withcaller_WithExtraKRAS_noDup.maf", data.table = F)
...
#traceback()
#https://wustl.app.box.com/folder/68187332272
#https://confluence.ris.wustl.edu/pages/viewpage.action?spaceKey=DL&title=CPTAC+Phospho-signaling+Project
#https://drive.google.com/drive/folders/1pJ3MHf8Vxq-XMQt2_DCoIJ2xzQqm9Xy3
